...
class CRFConstituency(nn.Module):
    ...
    def inside(self, scores, mask):
        """
        Calculate log partition function for CRF tree.
        ** Only the unbatched version **
        """
        batch_size, seq_len, seq_len, n_labels = scores.shape
        ...
        for d in range(2, seq_len+1):
         # define the offset variable for convenience
         offset=d-1

         # ** NO need for mask in batch_size = 1 implementation **
         # diag_mask = mask.diagonal(offset)

         for i in range(0, seq_len-d+1):
             j=i+offset
             span_ij = log_sum_exp(scores[i, j, :].view(1, -1))
             for k in range(i, j): # for each split point of span
                 # print("rest sum", s[i, k, 0], s[k+1, j, 0], s[i, k, 0] + s[k+1, j, 0])
                 t = torch.Tensor([s[i, j, 0], span_ij + s[i, k, 0] + s[k+1, j, 0]])
                 s[i, j, 0] = log_sum_exp(t.view(1, -1))
                 print(i, j, "s_ij", s[i, j, 0])
         return s
